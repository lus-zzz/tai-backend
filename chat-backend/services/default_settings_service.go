...
// NewDefaultSettingsService 创建默认配置服务
func NewDefaultSettingsService() *DefaultSettingsService {
	configDir := "config"
...
func (s *DefaultSettingsService) getBuiltinDefaults() *models.DefaultSettings {
...
func (s *DefaultSettingsService) GetDefaultSettings() *models.DefaultSettings {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *DefaultSettingsService) UpdateDefaultSettings(settings *models.DefaultSettings) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *DefaultSettingsService) ResetToDefaults() error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// load 从文件加载配置
func (s *DefaultSettingsService) load() error {
...
// save 保存配置到文件
func (s *DefaultSettingsService) save() error {
	if s.settings == nil {
		return nil
	}
